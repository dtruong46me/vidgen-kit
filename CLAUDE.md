...
| C. Render | Trông như thế nào? | `studio/` | Không |
...
- **Mỗi từ đúng MỘT nghĩa.** Bảng từ đầy đủ ở đầu `Makefile`; mấy cặp hay lẫn:
  *soạn* (Python sinh nguyên liệu: mp3 + build.json) ≠ *render* (Remotion vẽ ra
  MP4, ảnh bìa, ảnh tĩnh); *kiểm* (MÁY kết luận đạt/hỏng) ≠ *soát* (MẮT NGƯỜI
  kết luận: trang soát, ảnh tĩnh, romaji); *lời đăng* (chữ dán vào bài đăng,
  trường `caption`, file `caption.txt`) ≠ *caption* (chữ TRÊN HÌNH); *kho clip*
  (các file clip) ≠ *sổ clip* (`library/shots.json`) ≠ *thư viện* (thư viện
  Python/Node của `make setup`); *gói* chỉ là việc của `make export`. Đơn vị
  đếm của mọi lệnh là *ngày*. Đừng dùng "dựng" — nó từng mang cả ba nghĩa soạn,
  render lẫn chụp ảnh bìa, và đó chính là cái lộn xộn đã dọn.
- **Cài thư viện Python bằng `make setup`, đừng bao giờ gõ `pip install` trần.**
  Máy chạy dây chuyền có hơn một Python (conda base và python của codespace). `pip` trần trỏ
...
- **Bảng lệnh chỉ viết MỘT chỗ: dòng `#.` ngay trên mỗi target.** `make help`
  đọc thẳng các dòng `#:` (tên nhóm) và `#.` (`cách gõ | mô tả`) từ Makefile;
  `##` là giải thích dài, không in ra. Bản trước có ba chỗ mô tả cùng một lệnh
  (khối đầu file, target `help`, comment trên target), lệch nhau 13/20 dòng.
  Thêm lệnh mới thì thêm một dòng `#.`, đừng viết bảng ở chỗ nào khác.
- **Tên lệnh theo ba luật.** `<việc>` làm cho một ngày hoặc một khoảng ngày
  (`build`, `video`, `check`, `export`, `release`); `<việc>-all` làm cho MỌI
  ngày còn thiếu và không nhận ngày (`build-all`, `video-all`, `export-all`);
  `<sổ>-list` chỉ IN một cuốn sổ ra (`shots-list`, `bank-list`). Tên lệnh là tên
  thành phẩm khi có thể: `build` ra build.json, `video` ra MP4. Tên cũ
  (`content`, `content-all`, `all`, `shots`, `assets`, `bank`) còn chạy qua
  macro `renamed` — in một dòng nhắc rồi gọi tên mới — cho tới khi quen tay thì
  xoá hẳn nhóm "TÊN CŨ" ở cuối Makefile.
- **`make clean` và `make clean-all` là hai mức xoá, vì hai thứ đắt khác hẳn
  nhau.** `clean` xoá nguyên liệu (build.json, `.cache.json`, mp3) — soạn lại
  mất vài giây. `clean-all` xoá thêm `out/`, tức MỌI MP4 — render lại mất hàng
  giờ. Tách lệnh chứ không hỏi "gõ yes", vì hỏi thì script chạy tự động sẽ treo.
- **Remotion phải chạy với cwd là `studio/`** (nơi có `package.json`). Đường dẫn
  ...
  chỉ được viết ở `pipeline/render.py`, kể cả cho `make studio`
  (`python3 -m pipeline.render studio`). Makefile không tự ghép đường dẫn nào:
  cần biết ngày nào chưa soạn, chưa có MP4, hay vừa soạn gần nhất thì hỏi
  `python3 -m pipeline.paths --list --unbuilt` / `--unrendered` / `--latest-built`.
  Ngoại lệ duy nhất là `rm` trong `make clean` — xoá thì phải gọi đúng tên.
...
- **Ảnh bìa chụp lúc caption câu 1 VÀO XONG** (`thumbnailFrame`, do `timeline.py`
  ...
  cùng nhịp với caption. `make video` render ảnh bìa ngay sau MP4, `make export`
  render nốt cho ngày nào còn thiếu; `make release` là video → check → export,
  cho một ngày hoặc cả khoảng ngày — một lệnh từ kịch bản đã viết tới gói đăng.
- **Lời đăng là chữ ĐỂ ĐĂNG, không phải chữ để vẽ.** Nó nằm ở trường `caption`
  trong kịch bản (tên theo cách TikTok/Reels gọi ô chữ bài đăng), không hiện
  trong video; nó đi ra `out/<ngày>/caption.txt`. Ngày
  ...
  máy bỏ đi và `make build` kêu một dòng, không in hai emoji liền nhau. Bỏ
  trống `caption` thì máy mượn câu tiếng Việt cuối cùng và kêu một dòng — chạy
  được nhưng nhạt.
- **`caption` (lời đăng) là NỘI DUNG, `hashtags` là CÀI ĐẶT.** `caption` nằm trong
...
- **`make export` chỉ ĐỌC, trừ đúng một thứ: ẢNH BÌA.** Nó gói
  ...
  Render không được thì kêu một dòng rồi gói tiếp, không chặn.
- **Một ngày là một thư mục `out/<ngày>/`, đủ thứ để ĐĂNG.** `render.py` render
  MP4 và ảnh bìa thẳng vào đó; `export.py` viết lời đăng, lời Nhật–Việt,
  metadata và chép giọng đọc vào cạnh. Không chép video qua lại: bản trước render ra
...
- **`remotion.config.ts` ghì concurrency về 1 vì máy render thiếu RAM.** Không phải
...
- **Studio phải mở bằng dữ liệu thật.** `make studio` truyền `--props` trỏ vào
  ...
  đường lui khi chưa soạn ngày nào.
...
- **Nhạc nền CÓ commit, clip nền thì KHÔNG.** Nhạc nhẹ và không tải lại được
  bằng lệnh, nên nó vào git. Clip nền thì ngược lại: kho 52 clip dọc
  1080×1920 nặng 1,6 GB, mà `library/shots.json` ghi đủ nguồn và id nên
  `make shots-get` tải lại được từng cái — sổ vào git, file thì không. Vì vậy
  máy mới clone về sẽ thấy `make shots-list` kêu "thiếu file" cho tới khi tải lại.
  ...
  `make build` soạn lại được trong vài giây.
...
- **Kho clip hiện có 52 clip, 7 tag chính:** `tea`, `walk`, `rain`, `flower`,
...
- **Đừng ghi `calm` cạnh một tag chủ đề trong kịch bản.** `shots.matches()` ghép
  ...
  khớp gần hết kho clip và bộ chọn rơi về thứ tự trong sổ, tức toàn clip trà.
  Chỉ giữ `calm` khi kịch bản thật sự không có chủ đề hình ảnh nào.
- **Bộ chọn clip phải tất định.** Không random. Cùng kịch bản, cùng kho clip thì
  phải ra cùng kết quả, nếu không thì `make build` chạy hai lần ra hai file
...
- **Tải bản dọc vừa đủ 1080px, đừng lấy bản rộng nhất.** Video đích chỉ
  ...
  con số API trả về, `make shots-list` đo lại bằng ffprobe mới là số thật.
- **Khoá API đọc từ `.env` ở gốc repo, không bao giờ vào git.** `pipeline/env.py`
  ...
  trước nếu chưa có (đã có thì KHÔNG đè). Không có khoá thì mọi lệnh soạn và render
...
- **Mọi kịch bản mở đầu bằng MỘT dòng: `今日は、<ngày>です。おはようございます。`.**
  ...
  văn; `make bank-list` và `make new` cảnh báo chỗ nào lệch.
...
pipeline/     Lớp A + B (Python)
  ...
  post.py       chữ cho BÀI ĐĂNG: lời đăng + hashtag (không vẽ lên video)
  ...
  paths.py      chỗ DUY NHẤT biết content/ và out/ bày ra sao; đọc khoảng ngày FROM/TO/MONTH;
                trả lời Makefile ngày nào chưa soạn / chưa có MP4 / vừa soạn gần nhất
  ...
  library.py    sổ clip — đọc/ghi library/shots.json, in và kiểm bằng `make shots-list`
  ...
  render.py     gọi Remotion — chỗ duy nhất biết quy ước cwd=studio/ và ../;
                cửa vào của make studio: python3 -m pipeline.render studio [ngày]
  ...
  check.py      make check — số đo MP4 + trang soát từng MÀN CHỮ
  export.py     make export — gói out/<ngày>/: lời đăng, lời Nhật–Việt, audio, metadata (video + ảnh bìa đã nằm sẵn)
...
content/      <ngày>/       MỘT NGÀY LÀ MỘT THƯ MỤC
                script.json   người viết, commit. KHÔNG còn trường romaji;
                              ...
                              `caption` (lời đăng — nội dung) và `hashtags` (cài đặt);
...
Makefile      mọi lệnh; bảng từ ngữ ở đầu file; `make help` in bảng lệnh từ dòng `#.`
README.md     TỔNG QUAN: sơ đồ dây chuyền, lệnh nào gộp bước nào, các cách đi
...
> Phần này là NHẬT KÝ nghiệm thu: giữ nguyên từ ngữ lúc viết ("dựng", "trang
> duyệt"…), chỉ tên lệnh được đổi sang tên hiện tại (`make build` là
> `make content` cũ, `make shots-list` là `make shots` cũ). Bộ từ hiện hành ở
> đầu `Makefile` và mục "Mỗi từ đúng MỘT nghĩa" phía trên.

**BƯỚC 0 tới 9 đã xong trọn vẹn.** `make build` gọi `python3 -m
...
> `pipeline/` xong, chạy `make build DAY=2026-08-20` và nhìn con số đó. Nó đổi
...
1. **Nguồn và giấy phép truy ngược được hết.** `library/shots.json` ghi đủ năm
   ...
   nhạc thì vẫn thiếu url và giấy phép, `make shots-list` còn kêu chỗ đó.
...
| D-7 | `scripts/check_assets.py` gọi ffprobe riêng, trái quy ước "probe.py là chỗ duy nhất" | Đóng — gộp vào `pipeline/library.py`, `make assets` thành bí danh của `make shots` (nay cả hai là tên cũ của `make shots-list`) |
| D-8 | Cả ba clip đều 25fps trong timeline 30fps — cảnh lia chậm hơi giật | **Thu hẹp.** 49 clip thêm vào sau này đều đúng 30fps, nên hàng mới không dính. Ba clip gốc vẫn 25fps và `make shots-list` vẫn kêu — xem T-3 |
...
1. **Kịch bản mới không cần khoá.** `make new DAY=2026-09-10` lấy mục
   `ame-no-oto` từ ngân hàng, qua `script.py`, và `make build` dựng được ngay:
...
4. **Bố cục cũ không im lặng hỏng.** Còn sót `content/<ngày>.json` phẳng thì
   `make build` dừng lại và in đúng câu lệnh `git mv` cần gõ, chứ không báo
...
4. **Máy cắt được gần hết, và chỗ không cắt được thì nói ra.** 6 câu được cắt
   ...
   nên `make build` kêu đúng câu đó, và nó được chia tay bằng dạng danh sách —
...
hiện tại không cảnh nào phải loop — `make shots-list` kiểm tra lại được.
